<html>
    <body>
        <?php
        function get_bool_array($arr){
            if(empty($arr)){
                return array();
            }
            $last = $arr[count($arr)-1];
            $outarr = array();
            $outarr = array_fill(0,$last+1,false);
	    ...
	    //establish connection
            $conn=connect();
            echo "<form name='submit1' method='POST' action='subscribe.php'>";
            ...
            foreach($rows as $row){
                if(is_subscribed($conn,$row['SENSOR_ID'],$pid)){
                    echo "<input type='checkbox' name='sensors[]' value='".$row['SENSOR_ID']."' checked>".$row['LOCATION'].'<br>';
                } else {
                    echo "<input type='checkbox' name='sensors[]' value='".$row['SENSOR_ID']."'>".$row['LOCATION'].'<br>';
                }
            }
            oci_close($conn);
